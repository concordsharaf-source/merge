...
function bindEvents() {
  ...
  root.querySelectorAll("[data-category-action]").forEach((button) => button.addEventListener("click", () => { const category = button.dataset.category || "الكل"; if (button.dataset.categoryAction === "product-category") state.productCategory = category; else state.inventoryCategory = category; render(); }));
