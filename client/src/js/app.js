...
function salesMarkup() {
  ...
  <div class="cart-total"><div class="cart-pay-grid">${state.cart.length ? `<button class="cart-pay-btn is-cash" type="button" data-action="checkout" data-checkout-method="نقدي" title="إتمام البيع نقدًا">${msymbol("payments", "text-[19px]")}<span>دفع نقدي (كاش)</span></button><button class="cart-pay-btn is-transfer" type="button" data-action="checkout" data-checkout-method="تحويل" title="إتمام البيع بتحويل">${msymbol("account_balance_wallet", "text-[19px]")}<span>تحويل / محفظة</span></button>` : ""}</div><button class="button button--primary ${state.cart.length ? "checkout-launch" : "button--wide"}" data-action="checkout" ${state.cart.length ? "" : "disabled"}>${state.cart.length ? `<span class="checkout-launch__total"><small>إجمالي الفاتورة المطلوب</small><strong data-cart-subtotal>${money(totals.total)}</strong></span><span class="checkout-launch__go"><span>تأكيد وإصدار الفاتورة</span>${msymbol("receipt_long", "text-[22px]")}</span>` : `إتمام البيع ${msymbol("receipt_long", "text-[20px]")}`}</button></div></aside></section><section class="sales-bottom-action"><div><span class="eyebrow">سجل المبيعات</span><strong>فواتير المبيعات</strong><small>اعرض الفواتير المحفوظة وابحث عنها وراجع تفاصيل كل فاتورة.</small></div><button class="button button--primary" data-action="navigate" data-view="invoices">${msymbol("receipt_long", "text-[22px]")}<span>الانتقال إلى فواتير المبيعات</span></button></section>`;
}
