...
import { calculateDiscountAmount, calculatePackagePurchase, calculateSaleTotals, calculateTransferCollections, dateKey, expiryProgress, invoiceNumber, normalizeCashierDiscountLimit, nowIso, roundMoney, stockStatus, toNumber } from "./domain.js";
...
function expiryStatus(product, today = dateKey()) {
  ...
  return { expiryDate, days, ratio: progress?.ratio ?? null, tone: days <= 30 ? "danger" : "warning", label: days < 0 ? "منتهي" : progress ? `استهلك ${Math.min(100, Math.round(progress.ratio * 100))}% من الصلاحية` : days <= 30 ? "ينتهي خلال شهر" : days <= 90 ? "ينتهي خلال 3 أشهر" : `متبقي ${amount(days)} يوم على الانتهاء` };
}
...
function expiryMeterMarkup(product) { const progress = expiryProgress({ productionDate: product?.nearestProductionDate, expiryDate: product?.nearestExpiryDate }); const fallbackExpiry = String(product?.nearestExpiryDate || ""); if (!progress && /^\d{4}-\d{2}-\d{2}$/.test(fallbackExpiry)) { const daysLeft = Math.ceil((new Date(`${fallbackExpiry}T00:00:00`).getTime() - new Date(`${dateKey()}T00:00:00`).getTime()) / 86400000); const remaining = Math.max(0, Math.min(1, daysLeft / 365)); const tone = daysLeft <= 30 ? "danger" : daysLeft <= 90 ? "warning" : "safe"; return `<div class="expiry-meter expiry-meter--${tone}" title="المتبقي ${Math.round(remaining * 100)}% من سنة الصلاحية التقديرية"><span style="width:${Math.round(remaining * 100)}%"></span></div>`; } if (!progress) return `<div class="expiry-meter expiry-meter--empty" title="لا توجد تواريخ إنتاج وانتهاء مكتملة"><span></span></div>`; const consumed = Math.max(0, Math.min(1, progress.ratio)); const remaining = 1 - consumed; const tone = consumed >= 1 ? "danger" : consumed >= 0.85 ? "warning" : "safe"; return `<div class="expiry-meter expiry-meter--${tone}" title="المتبقي ${Math.round(remaining * 100)}% من مدة الصلاحية"><span style="width:${Math.round(remaining * 100)}%"></span></div>`; }
  ...
  <section class="pt-space-sm pb-space-xs flex flex-col gap-space-xs">
    <div class="flex items-center justify-between gap-2 bg-surface-container-low p-space-sm rounded-xl">
      ...
      <div class="flex items-center gap-1 shrink-0">
        ...
        <button aria-label="ابحث في التطبيق" title="ابحث في التطبيق" class="w-9 h-9 flex items-center justify-center rounded-lg bg-surface-container text-on-surface-variant active:scale-95 transition-transform" type="button" data-action="open-app-search">${msymbol("search", "text-[20px]")}</button>
      </div>
    </div>
...
const cartStyle = () => (state.settings?.cartStyle === "bar" ? "bar" : "detailed");
function cartBreakdownMarkup(totals) {
  if (!state.cart.length) return "";
  return `<div class="cart-breakdown"><div class="cb-row"><span>المجموع الفرعي</span><strong>${money(totals.subtotal)}</strong></div><div class="cb-row cb-discount"><span>خصم ترويجي <button class="text-button cb-edit" type="button" data-action="checkout" title="تعديل الخصم في مراجعة الفاتورة">تعديل</button></span><strong class="cb-discount__value">${toNumber(totals.discount) ? `- ${money(totals.discount)}` : money(0)}</strong></div><div class="cb-row cb-total"><span>الإجمالي النهائي</span><strong>${money(totals.total)}</strong></div></div>`;
}
function cartPaytypeMarkup() {
  return `<div class="cart-paytype"><span class="cart-paytype__label">نوع العملية المحاسبية</span><div class="cart-paytype__grid"><button class="cart-paytype__btn is-cash" type="button" data-action="checkout" data-checkout-method="نقدي">${msymbol("payments", "text-[18px]")}<span>بيع نقدي</span></button><button class="cart-paytype__btn is-credit" type="button" data-action="checkout" data-checkout-method="آجل">${msymbol("account_balance_wallet", "text-[18px]")}<span>بيع آجل (دين)</span></button></div></div>`;
}
function salesMarkup() {
  ...
  <div class="cart-total">${cartStyle() === "detailed" ? cartBreakdownMarkup(totals) : ""}${cartStyle() === "detailed" && state.cart.length ? cartPaytypeMarkup() : ""}<div class="cart-pay-grid">${state.cart.length ? `<button class="cart-pay-btn is-cash" type="button" data-action="checkout" data-checkout-method="نقدي" title="إتمام البيع نقدًا">${msymbol("payments", "text-[19px]")}<span>${cartStyle() === "detailed" ? "كاش بالدرج" : "دفع نقدي (كاش)"}</span></button><button class="cart-pay-btn is-transfer" type="button" data-action="checkout" data-checkout-method="تحويل" title="إتمام البيع بتحويل">${msymbol("account_balance_wallet", "text-[19px]")}<span>${cartStyle() === "detailed" ? "الكريمي / ون كاش" : "محفظة / الكريمي"}</span></button>` : ""}</div><button class="button button--primary ${state.cart.length ? "checkout-launch" : "button--wide"}" data-action="checkout" ${state.cart.length ? "" : "disabled"}>${state.cart.length ? `<span class="checkout-launch__total"><small>إجمالي الفاتورة المطلوب</small><strong data-cart-subtotal>${money(totals.total)}</strong></span><span class="checkout-launch__go"><span>${cartStyle() === "detailed" ? "تأكيد وطباعة الفاتورة" : "تأكيد وإصدار الفاتورة"}</span>${msymbol("receipt_long", "text-[22px]")}</span>` : `إتمام البيع ${msymbol("receipt_long", "text-[20px]")}`}</button></div></aside></section><section class="sales-bottom-action"><div><span class="eyebrow">سجل المبيعات</span><strong>فواتير المبيعات</strong><small>اعرض الفواتير المحفوظة وابحث عنها وراجع تفاصيل كل فاتورة.</small></div><button class="button button--primary" data-action="navigate" data-view="invoices">${msymbol("receipt_long", "text-[22px]")}<span>الانتقال إلى فواتير المبيعات</span></button></section>`;
}
...
function purchasesMarkup() {
  ...
  <section class="pp-hero"><div class="pp-hero__value"><span>إجمالي المشتريات</span><strong>${money(state.analytics?.purchases.total || 0)}</strong></div><div class="pp-hero__minis"><div class="pp-mini"><span>فواتير الشراء</span><strong>${amount(state.purchases.length)} فاتورة</strong></div><div class="pp-mini"><span>متوسط الفاتورة</span><strong>${money(state.purchases.length ? toNumber(state.analytics?.purchases.total || 0) / state.purchases.length : 0)}</strong></div></div></section>
  ...
  <section class="panel invoice-list pp-list">${state.purchases.length ? state.purchases.map((purchase) => `<button class="invoice-row" data-action="open-purchase" data-id="${purchase.id}"><div class="invoice-row__mark invoice-row__mark--purchase">${msymbol("local_shipping", "text-[20px]")}</div><div class="invoice-row__main"><strong>${purchase.invoiceNumber}</strong><small>${escapeHtml(purchase.supplierName)} · ${dateTime(purchase.date)} · ${amount((purchase.items || []).length)} صنف</small></div><strong>${money(purchase.total)}</strong>${icon("arrow", 18)}</button>`).join("") : emptyState("لا توجد فواتير شراء", "سجّل أول فاتورة شراء لزيادة المخزون، مع المورد أو بدونه.", "new-purchase")}</section>`;
}
...
function reportsHeroMarkup(data) {
  const profit = toNumber(data.profit?.netProfit);
  return `<section class="rp-hero"><div class="rp-hero__value"><span>صافي الربح</span><strong>${money(data.profit?.netProfit || 0)}</strong><small>${profit < 0 ? "خسارة ضمن الفترة المحددة" : profit > 0 ? "بعد خصم التكلفة والمصروفات" : "لا ربح ولا خسارة ضمن الفترة"}</small></div><div class="rp-hero__minis"><div class="rp-mini"><span>صافي المبيعات</span><strong>${money(data.profit?.netSales || 0)}</strong></div><div class="rp-mini"><span>تكلفة البضاعة</span><strong>${money(data.profit?.netCostOfGoods || 0)}</strong></div><div class="rp-mini"><span>إجمالي المصروفات</span><strong>${money(data.expenses?.total || 0)}</strong></div></div></section>`;
}
...
function reportsMarkup() {
  ...
  ${reportsHeroMarkup(data)}
  <section class="report-grid rp-cards"><article class="panel report-card"><div class="rp-card__top"><span class="rp-card__icon">${msymbol("point_of_sale", "text-[20px]")}</span><span class="eyebrow">تقرير المبيعات</span></div><h2 class="${toNumber(data.sales.total) < 0 ? "is-negative" : ""}">${money(data.sales.total || 0)}</h2><div><span>الفواتير</span><strong>${amount(data.sales.invoices || 0)}</strong></div><div><span>الخصومات</span>${signedMoney(data.sales.discounts || 0)}</div><div><span>مرتجع البيع</span>${signedMoney(data.sales.returns || 0)}</div></article><article class="panel report-card"><div class="rp-card__top"><span class="rp-card__icon">${msymbol("shopping_cart", "text-[20px]")}</span><span class="eyebrow">تقرير المشتريات</span></div><h2 class="${toNumber(data.purchases.net) < 0 ? "is-negative" : ""}">${money(data.purchases.net || 0)}</h2><div><span>الفواتير</span><strong>${amount(data.purchases.invoices || 0)}</strong></div><div><span>المنتجات المشتراة</span><strong>${amount(data.purchases.products || 0)}</strong></div><div><span>مرتجع الشراء</span>${signedMoney(data.purchases.returns || 0)}</div></article><article class="panel report-card"><div class="rp-card__top"><span class="rp-card__icon">${msymbol("receipt_long", "text-[20px]")}</span><span class="eyebrow">المصروفات حسب النوع</span></div>${categories.length ? categories.map(([category, total]) => `<div><span>${escapeHtml(category)}</span>${signedMoney(total)}</div>`).join("") : `<p>لا توجد مصروفات ضمن الفترة.</p>`}</article><article class="panel report-card report-card--profit"><div class="rp-card__top"><span class="rp-card__icon">${msymbol("monitoring", "text-[20px]")}</span><span class="eyebrow">معادلة الربح</span></div><div><span>المبيعات</span>${signedMoney(data.profit.netSales || 0)}</div><div><span>− تكلفة البضاعة</span>${signedMoney(data.profit.netCostOfGoods || 0)}</div><div><span>= الربح الإجمالي</span>${signedMoney(data.profit.grossProfit || 0)}</div><div><span>− المصروفات</span>${signedMoney(data.expenses.total || 0)}</div><div class="report-card__final"><span>= صافي الربح</span>${signedMoney(data.profit.netProfit || 0)}</div></article></section>
...
function settingsContactMarkup() {
  return `<section class="panel settings-contact-card"><span class="eyebrow">تواصل مع المصمم</span><h2>نسعد باستقبال ملاحظاتكم واقتراحاتكم</h2><p>لتحسين تجربة استخدام حسابي وتطويرها باستمرار.</p><a class="settings-contact-card__phone" href="https://wa.me/967770388100" target="_blank" rel="noopener noreferrer">واتساب: +967770388100</a><a class="settings-contact-card__email" href="mailto:concordsharaf@gmail.com">concordsharaf@gmail.com</a><small>تصميم شرف غالب قحطان · الجمهورية اليمنية · إصدار الواجهة v45</small></section>`;
}
...
function loginMarkup() {
  ...
  return `<main class="setup-page login-page entry-page"><div class="entry-wrap"><section class="entry-brand"><img src="${storeLogoUrl()}" alt="شعار ${escapeHtml(state.settings?.storeName || "المتجر")}" /><div><strong>حسابي | Hesabi <span class="entry-version">v2.4</span></strong><small>${escapeHtml(storeDisplayName())}</small></div><p class="entry-offline">${msymbol("verified_user", "text-[16px]")}<span>يعمل محليًا 100% دون إنترنت • حفظ فوري داخل جهازك (IndexedDB)</span></p></section><section class="entry-setup entry-login"><div class="entry-setup__head"><span class="entry-setup__icon">${msymbol("person", "text-[20px]")}</span><div><span class="eyebrow">مرحبًا بعودتك</span><h2>الدخول إلى الحساب</h2></div></div><form id="login-form"><div class="entry-field"><label for="login-username">اسم المستخدم</label><div class="entry-control">${msymbol("person", "text-[20px]")}<input id="login-username" name="username" dir="ltr" autocomplete="username" required minlength="3" maxlength="30" autofocus placeholder="مثال: admin" /></div></div><div class="entry-field"><label for="login-pin">كلمة المرور</label><div class="entry-control entry-pin-control">${msymbol("key", "text-[20px]")}<input id="login-pin" name="pin" type="password" dir="ltr" autocomplete="current-password" required minlength="4" maxlength="64" placeholder="••••" /><button class="entry-eye" type="button" data-action="toggle-setup-pin" aria-label="إظهار كلمة المرور">${msymbol("visibility", "text-[18px]")}</button></div></div><button class="button button--primary button--wide entry-submit" type="submit"><span>دخول إلى حسابي</span>${msymbol("arrow_back", "text-[22px]")}</button></form><button type="button" class="login-recovery-link" data-action="open-phone-recovery">نسيت كلمة المرور المحلية؟ اطلب استردادًا برقم الجوال</button><button type="button" class="login-recovery-link" data-action="cloud-password-reset">استعادة كلمة مرور النسخ عبر البريد الإلكتروني</button><button type="button" class="button button--secondary button--wide login-home-button" data-action="setup-home">الرئيسية · اختيار نوع المتجر والبدء من جديد ${icon("arrow", 18)}</button><div class="account-hints"><strong>الحسابات المتاحة</strong>${users.map((account) => `<button type="button" class="account-hint" data-action="fill-login" data-username="${escapeHtml(account.username)}"><span>${escapeHtml(account.name)}</span><small>${escapeHtml(account.username)} · ${roleLabel(account.role)}</small></button>`).join("")}</div><small class="offline-note"><span class="presence-dot"></span>يسجل الدخول محليًا ولا يحتاج اتصالًا بالإنترنت</small><small class="build-stamp">إصدار الواجهة: v50 · 15 سبتمبر 2026</small></section></div></main>`;
}
...
function setupMarkup() {
  const currencyCards = CURRENCIES.map((currency) => `<label class="currency-card"><input class="sr-only" name="currency" type="radio" value="${currency.code}" ${currency.code === DEFAULT_CURRENCY_CODE ? "checked" : ""} /><span class="currency-card__body"><strong>${escapeHtml(currency.label)}</strong><small>${currency.code === DEFAULT_CURRENCY_CODE ? "الافتراضي في السوق" : currency.code === "SAR" ? "للمعاملات الموازية" : currency.code}</small></span></label>`).join("");
  return `<main class="setup-page entry-page"><div class="entry-wrap"><section class="entry-brand"><img src="${storeLogoUrl()}" alt="حسابي" /><div><strong>حسابي | Hesabi <span class="entry-version">v2.4</span></strong><small>نظام الكاشير الذكي والمخزون الفوري للبقالات والمتاجر</small></div><p class="entry-offline">${msymbol("verified_user", "text-[16px]")}<span>يعمل محليًا 100% دون إنترنت • حفظ فوري داخل جهازك (IndexedDB)</span></p></section><section class="entry-restore"><span class="entry-restore__icon">${msymbol("settings_backup_restore", "text-[22px]")}</span><div><strong>هل لديك نسخة سابقة؟</strong><small>استعادة بياناتك السابقة فورًا عبر ملف JSON</small></div><label class="button button--primary" for="setup-restore-file">${msymbol("upload", "text-[18px]")}<span>استعادة الآن</span></label><input id="setup-restore-file" type="file" accept="application/json,.json" hidden /></section><section class="entry-setup"><div class="entry-setup__head"><span class="entry-setup__icon">${msymbol("storefront", "text-[20px]")}</span><div><h2>التهيئة السريعة للمتجر</h2><small>أدخل البيانات الأساسية لطباعة الفواتير</small></div></div><button class="button button--primary button--wide setup-register-button entry-launch" type="button" data-action="open-setup-form">تسجيل جديد ${icon("arrow", 18)}</button><form id="setup-form" hidden><div class="entry-field"><label for="setup-store-name"><span>اسم البقالة / المتجر <i class="req">*</i></span><small>يظهر في رأس الفاتورة</small></label><div class="entry-control">${msymbol("shopping_basket", "text-[20px]")}<input id="setup-store-name" name="storeName" dir="rtl" required maxlength="60" placeholder="مثلاً: تموينات الرضا السريعة" autofocus /></div></div><div class="entry-field"><label for="setup-business">نوع النشاط التجاري</label><div class="entry-control">${msymbol("category", "text-[20px]")}<select id="setup-business" name="businessType" required>${BUSINESS_TYPES.map((type) => `<option value="${type}">${type}</option>`).join("")}</select><span class="entry-control__chev">${msymbol("expand_more", "text-[18px]")}</span></div></div><div class="entry-field"><div class="entry-field__head"><span>العملة الأساسية للحسابات</span><small class="entry-link">دعم متعدد العملات</small></div><div class="currency-grid">${currencyCards}</div></div><div class="entry-pinbox"><div class="entry-field__head"><span>صلاحية الدخول الأولية</span><span class="entry-rolechip">مدير متجر</span></div><div class="entry-field"><label for="setup-username">اسم المستخدم</label><div class="entry-control">${msymbol("person", "text-[20px]")}<input id="setup-username" name="username" dir="ltr" required minlength="3" maxlength="30" autocomplete="username" placeholder="مثال: ahmed" /></div></div><div class="entry-field"><label for="setup-account-name">اسم صاحب الحساب</label><div class="entry-control">${msymbol("badge", "text-[20px]")}<input id="setup-account-name" name="accountName" dir="rtl" required maxlength="60" autocomplete="name" placeholder="مثال: أحمد محمد" /></div></div><div class="entry-field"><label for="setup-pin">رمز القفل السري (PIN)</label><div class="entry-control entry-pin-control">${msymbol("key", "text-[20px]")}<input id="setup-pin" name="pin" type="password" dir="ltr" required minlength="4" maxlength="64" autocomplete="new-password" placeholder="4 أحرف أو أرقام على الأقل" /><button class="entry-eye" type="button" data-action="toggle-setup-pin" aria-label="إظهار الرمز">${msymbol("visibility", "text-[18px]")}</button></div></div><div class="entry-field"><label for="setup-pin-confirm">تأكيد الرمز السري</label><div class="entry-control">${msymbol("key", "text-[20px]")}<input id="setup-pin-confirm" name="pinConfirm" type="password" dir="ltr" required minlength="4" maxlength="64" autocomplete="new-password" placeholder="أعد كتابة الرمز" /></div></div><p class="entry-note">${msymbol("info", "text-[15px]")}<span>اختر رمزًا لا يقل عن 4 أحرف — ستدخل به يوميًا.</span></p></div><button class="button button--primary button--wide entry-submit" type="submit"><span>دخول وبدء العمل في المتجر</span>${msymbol("arrow_back", "text-[22px]")}</button></form><div class="entry-features"><div>${msymbol("wifi_off", "text-[20px]")}<strong>أوفلاين دائم</strong><small>لا ينقطع أبدًا</small></div><div>${msymbol("barcode_scanner", "text-[20px]")}<strong>كاميرا الكود</strong><small>مسح فوري</small></div><div>${msymbol("print", "text-[20px]")}<strong>طابعة بلوتوث</strong><small>80mm و 58mm</small></div></div><button class="button button--secondary button--wide" type="button" data-action="cloud-restore-start">استعادة من نسخة سحابية ${msymbol("cloud_download", "text-[19px]")}</button><a class="button button--secondary button--wide" href="./user-guide.html" target="_blank" rel="noopener">دليل الاستخدام ${icon("arrow", 18)}</a><small class="offline-note"><span class="presence-dot"></span>يحفظ محليًا ويظل متاحًا بعد أول تحميل</small></section></div></main>`;
}
...
async function handleActionUnsafe(event) {
  ...
  if (action === "set-cart-style") { setCartStylePreference(event.currentTarget.dataset.cartStyle); return; }
  ...
  if (action === "toggle-setup-pin") { toggleSetupPinVisibility(event.currentTarget); return; }
...
async function setCartStylePreference(style) {
  try {
    if (style !== "detailed" && style !== "bar") return;
    await db.saveSettings({ ...state.settings, cartStyle: style });
    state.settings = await db.getSettings();
    render();
    showToast(style === "bar" ? "شكل سلة البيع: شريط مختصر" : "شكل سلة البيع: مفصلة");
  } catch (error) { showToast(error.message, "error"); }
}
...
function toggleSetupPinVisibility(button) {
  const form = button?.closest("form");
  if (!form) return;
  const fields = form.querySelectorAll('input[name="pin"], input[name="pinConfirm"]');
...
function openSaleSuccessDialog(sale) {
  const change = roundMoney(toNumber(sale.paidAmount) - toNumber(sale.total));
  const rows = sale.paymentType === "آجل"
    ? `<div><span>المبلغ المدفوع:</span><strong>${money(sale.paidAmount)}</strong></div><div><span>قيمة الفاتورة:</span><strong>${money(sale.total)}</strong></div><div class="sale-success__debt"><span>المتبقي على العميل:</span><strong>${money(sale.remainingAmount)}</strong></div>`
    : `<div><span>المبلغ المدفوع كاش:</span><strong>${money(sale.paidAmount)}</strong></div><div><span>قيمة الفاتورة:</span><strong>${money(sale.total)}</strong></div><div><span>المتبقي للعميل:</span><strong>${money(Math.max(0, change))}</strong></div>`;
  openDialog(`<div class="sale-success"><div class="sale-success__badge">${msymbol("check", "text-[36px]")}</div><h3>تم حفظ الفاتورة بنجاح!</h3><p class="dialog__subtext">رقم العملية: <bdi>#${escapeHtml(sale.invoiceNumber)}</bdi> · ${escapeHtml(storeDisplayName())}</p><div class="sale-success__rows">${rows}</div><div class="sale-success__actions"><button class="button button--primary" data-action="print-invoice-direct" data-id="${sale.id}">${msymbol("print", "text-[20px]")}<span>طباعة فورية (حراري)</span></button><button class="button button--secondary" data-dialog-close>${msymbol("add_shopping_cart", "text-[18px]")}<span>بدء فاتورة جديدة</span></button></div></div>`);
}
...
function openCheckoutDialog(presetMethod = "") {
  ...
  `;
  const overlay = openDialog(`<div class="stitch-checkout"><div class="dialog__head"><div><span class="eyebrow">إتمام البيع</span><h2>مراجعة الفاتورة</h2><p class="dialog__subtext">${initial.lines.length} أصناف · ${amount(initial.lines.reduce((sum, line) => sum + toNumber(line.quantity), 0))} قطعة</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${msymbol("close", "text-[20px]")}</button></div><div class="checkout-lines stitch-lines co-lines">${initial.lines.map((line) => `<div><span>${escapeHtml(line.name)} × ${amount(line.quantity)}</span><strong>${money(line.total)}</strong></div>`).join("")}</div><form id="checkout-form" class="form-grid stitch-form"><div class="co-card form-full"><div class="co-row"><span>المجموع الفرعي</span><strong>${money(initial.subtotal)}</strong></div><label class="co-row co-discount"><span>خصم ترويجي</span><input name="discount" type="text" inputmode="decimal" placeholder="0 أو 20%" value="" autocomplete="off" /></label><small class="field-hint">أدخل مبلغًا مثل 100 أو نسبة مثل 20%</small><div class="co-row co-row--total"><span>الإجمالي النهائي</span><strong id="checkout-total">${money(initial.total)}</strong></div></div><div class="delivery-charge-type delivery-compact form-full stitch-delivery"><label class="delivery-compact__amount">خدمة التوصيل<input name="deliveryFee" type="number" inputmode="decimal" min="0" step="0.01" placeholder="0" /></label><div class="delivery-compact__choices" role="radiogroup" aria-label="خدمة التوصيل على"><label class="delivery-choice delivery-choice--store"><input name="deliveryChargeType" type="radio" value="store" checked /> <span>على المحل</span></label><label class="delivery-choice delivery-choice--customer"><input name="deliveryChargeType" type="radio" value="customer" /> <span>على العميل</span></label></div></div>${paymentMethodToggle}<fieldset class="payment-type form-full stitch-paytype co-paytype"><legend>نوع العملية المحاسبية</legend><label><input name="paymentType" type="radio" value="نقدي" checked />${msymbol("payments", "text-[18px]")}<span>بيع نقدي</span></label><label><input name="paymentType" type="radio" value="آجل" />${msymbol("account_balance_wallet", "text-[18px]")}<span>بيع آجل (دين)</span></label></fieldset><label id="credit-customer-field" class="form-full stitch-field" hidden>العميل<select name="customerId"><option value="">اختر العميل</option>${state.customers.map((customer) => `<option value="${customer.id}">${escapeHtml(customer.name)}${toNumber(customer.balance) ? ` — رصيد ${money(customer.balance)}` : ""}</option>`).join("")}</select></label><div id="credit-due-date-field" class="form-full stitch-field" hidden><label>تاريخ استحقاق السداد (أجل اختياري)<input id="credit-due-date-input" name="dueDate" type="date" min="${dateKey()}" value="" /><small class="field-hint">حدد أجل السداد لتتبع الديون المتأخرة وإرسال التذكيرات.</small></label><div class="due-preset-chips"><button type="button" class="due-chip" data-due-days="7">+ 7 أيام</button><button type="button" class="due-chip" data-due-days="15">+ 15 يومًا</button><button type="button" class="due-chip" data-due-days="30">+ 30 يومًا</button></div></div>${cashCalcMarkup}<label class="form-full stitch-field">المبلغ المدفوع<input id="paid-amount" name="paidAmount" type="number" inputmode="decimal" min="0" max="${initial.total}" step="0.01" value="${initial.total}" required /></label><div class="credit-summary form-full stitch-credit" id="credit-summary" hidden><span>المبلغ المتبقي</span><strong id="remaining-amount">${money(0)}</strong><small id="payment-status">مدفوعة</small></div><div class="dialog__actions form-full co-submit-row"><button type="button" class="button button--secondary" data-dialog-close>رجوع</button><button type="submit" class="button button--primary checkout-submit co-submit"><span class="co-submit__go">${msymbol("task_alt", "text-[22px]")}<span>تأكيد وإصدار الفاتورة</span></span><span class="co-submit__total"><strong id="checkout-submit-total">${money(initial.total)}</strong>${msymbol("arrow_back", "text-[20px]")}</span></button></div></form></div>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
  const form = overlay.querySelector("#checkout-form");
  if (presetMethod === "آجل") form.paymentType.value = "آجل";
  ...
  const syncCheckout = () => {
    ...
    overlay.querySelector("#checkout-submit-total").textContent = money(totals.total);
