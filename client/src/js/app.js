...
function productsMarkup() {
  ...
  const products = state.products.filter((product) => (state.productCategory === "الكل" || (product.category || "أخرى") === state.productCategory) && (!query || [product.name, product.barcode, product.internalCode].some((value) => value?.toLocaleLowerCase("ar").includes(query))));
  return `${topbarMarkup("المنتجات", "ابحث بالاسم أو الباركود أو الكود الداخلي.", `<div class="topbar__actions"><button class="button button--secondary" data-action="navigate" data-view="inventory">${icon("layers", 17)}<span>المخزون</span></button><button class="button button--primary" data-action="new-product">${icon("plus", 18)}<span>إضافة منتج</span></button></div>`)}
  <section class="toolbar"><label class="search-field">${icon("search", 19)}<input id="product-search" dir="rtl" lang="ar" autocomplete="off" placeholder="ابحث عن منتج..." value="${escapeHtml(state.productQuery)}" /></label><button class="button button--secondary button--icon-text" data-action="open-scanner" data-mode="product">${icon("scan", 18)}<span>مسح باركود</span></button></section>${crossLinkMarkup("inventory", { eyebrow: "تنقّل سريع", title: "المخزون والكميات", subtitle: "الكميات، الجرد، تعديل الرصيد، وسجل الحركة", label: "فتح المخزون", glyph: "layers" })}<div class="category-toolbar"><strong>الأصناف (${amount(state.products.length)})</strong><div class="category-chips">${categoryButtons(state.productCategory, "product-category")}</div></div>
...
function inventoryMarkup() {
  ...
  const products = [...state.products].filter((product) => state.inventoryCategory === "الكل" || (product.category || "أخرى") === state.inventoryCategory).sort((a, b) => priority(a) - priority(b) || a.name.localeCompare(b.name, "ar"));
  ...
  <section class="inventory-summary"><div><span>إجمالي قيمة المخزون</span><strong>${money(state.dashboard.inventoryValue)}</strong></div><div><span>عدد المنتجات</span><strong>${amount(state.products.length)} منتج</strong></div><div><span>منخفض أو نافد</span><strong>${amount(state.dashboard.lowStock.length)} منتج</strong></div></section>${crossLinkMarkup("products", { eyebrow: "تنقّل سريع", title: "المنتجات والأسعار", subtitle: "إضافة منتج، تعديل الأسعار، والباركود", label: "فتح المنتجات", glyph: "package" })}<section class="reports-bottom-action inventory-audit-entry"><div><span class="eyebrow">الجرد والمراجعة</span><strong>الجرد المحاسبي الدوري</strong><small>راجع قيمة المخزون والأرصدة واعتمد لقطة شهرية أو سنوية للمقارنة.</small></div><button class="button button--primary" data-action="navigate" data-view="periodic-inventory">فتح الجرد المحاسبي ${icon("chart", 18)}</button></section><div class="category-toolbar"><strong>أصناف المخزون (${amount(products.length)})</strong><div class="category-chips">${categoryButtons(state.inventoryCategory, "inventory-category")}</div></div>
...
function periodicInventoryMarkup() {
  ...
  const historyMarkup = history.length ? history.map((audit) => `<article class="periodic-inventory-row"><button class="periodic-inventory-row__main" data-action="open-periodic-inventory" data-id="${audit.id}"><span class="periodic-inventory-row__icon">${icon("history", 19)}</span><span><strong>جرد ${periodicInventoryCycleLabel(audit.cycle)}</strong><small>${formatDate(audit.periodFrom)} — ${formatDate(audit.periodTo)} · اعتمد ${escapeHtml(audit.approvedByName || "الأدمن")}</small>${comparison(audit)}</span></button><strong class="periodic-inventory-row__amount ${toNumber(audit.metrics?.netPosition) < 0 ? "is-negative" : ""}">${money(audit.metrics?.netPosition)}</strong></article>`).join("") : `<div class="inline-empty">لم تحفظ أي لقطة جرد بعد. راجع الملخص الحالي ثم اعتمده لحفظه للمقارنة.</div>`;
  return `${topbarMarkup("الجرد المحاسبي الدوري", "لقطة محاسبية للفترة المختارة؛ الاعتماد لا يغيّر كميات المنتجات أو الصندوق أو الخزنة.", `<button class="button button--primary periodic-inventory-top-action" data-action="save-periodic-inventory">${icon("check", 17)}<span>اعتماد الجرد</span></button>`)}
  ...
  <section class="periodic-inventory-notice"><div>${icon("history", 20)}</div><div><strong>اللقطة لا تنشئ حركة جديدة</strong><span>تُقرأ الأرصدة الحالية للخزنة والديون والمخزون، وتُحسب نتائج الفترة من الفواتير والمصروفات المسجلة فقط.</span></div></section>
...
function openPhoneRecoveryDialog() {
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">استرداد بمراجعة المالك</span><h2>طلب استرداد رمز الدخول</h2><p class="dialog__subtext">أدخل رقم جوالك فقط. يصل الطلب إلى مالك المتجر، ثم يتواصل معك للتحقق قبل أن يمنحك رمزًا مؤقتًا. لا يُعاد تعيين أي رمز تلقائيًا.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><form id="phone-recovery-form" class="form-grid"><label>رقم الجوال<input name="phone" type="tel" dir="ltr" inputmode="tel" autocomplete="tel" required minlength="7" maxlength="20" placeholder="777123456" autofocus /></label><small class="recovery-phone-note">لا تحتاج إلى بريد إلكتروني أو مفتاح طوارئ.</small><div class="dialog__actions form-full"><button type="button" class="button button--secondary" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit">إرسال الطلب ${icon("phone", 17)}</button></div></form>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
...
function openCashierShiftTransferDialog(shiftId) {
  ...
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">ترحيل صندوق فرعي</span><h2>ترحيل وردية ${escapeHtml(shift.accountName || "الكاشير")}</h2><p class="dialog__subtext">اعتمد استلام مبلغ الجرد من صندوق الكاشير إلى الخزنة الرئيسية. لا يمكن تكرار هذا الترحيل.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><div class="cashier-shift-handover__received"><span>مبلغ الجرد المستلم للخزنة</span><strong>${money(shift.countedCash)}</strong></div><p class="scanner-session-note">${differenceNote}</p><div class="dialog__actions"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button id="confirm-cashier-shift-transfer" class="button button--primary" type="button">اعتماد الترحيل ${icon("check", 17)}</button></div>`);
...
async function openCashierShortageDeductionDialog(accountId) {
  ...
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">تسوية عجز الراتب</span><h2>خصم عجز ${escapeHtml(cashier.name)}</h2><p class="dialog__subtext">اختر ورديات العجز المرحّلة فقط. يُحفظ الخصم كسجل راتب مستقل ولا ينشئ مصروفًا أو حركة خزنة جديدة.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><form id="cashier-shortage-deduction-form" class="form-grid"><p class="form-full scanner-session-note">الراتب المتبقي بعد السلف والتسويات السابقة: <strong>${money(remainingSalary)}</strong></p><div class="form-full cashier-shortage-choices">${rows}</div><p id="cashier-shortage-deduction-preview" class="form-full field-hint"></p><div class="dialog__actions form-full"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit">اعتماد خصم العجز ${icon("check", 17)}</button></div></form>`);
...
function openPeriodicInventorySaveDialog() {
  ...
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">اعتماد لقطة محاسبية</span><h2>جرد ${periodicInventoryCycleLabel(state.auditCycle)}</h2><p class="dialog__subtext">سيُحفظ الملخص للفترة ${formatDate(range.from)} إلى ${formatDate(range.to)} للمقارنة لاحقًا. لن يغيّر الحفظ أي كمية أو رصيد.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><section class="periodic-inventory-save-summary"><div><span>الخزنة</span><strong>${money(data.cash.vaultBalance)}</strong></div><div><span>المخزون</span><strong>${money(data.inventory.cost)}</strong></div><div><span>الديون</span><strong>${money(data.receivables.customerDebt)}</strong></div><div><span>صافي الربح</span><strong class="${toNumber(data.performance.netProfit) < 0 ? "is-negative" : ""}">${money(data.performance.netProfit)}</strong></div></section><form id="periodic-inventory-save-form" class="form-grid"><label class="form-full">ملاحظة الجرد (اختيارية)<textarea name="notes" maxlength="240" placeholder="مثال: مراجعة نهاية الشهر"></textarea></label><div class="dialog__actions form-full"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit">اعتماد وحفظ الجرد ${icon("check", 17)}</button></div></form>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
...
function openPairingInviteDialog() {
  ...
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">ربط جهاز جديد</span><h2>إنشاء رمز اقتران لمرة واحدة</h2><p class="dialog__subtext">الرمز صالح لعشر دقائق ويُستخدم مرة واحدة فقط.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><form id="pairing-invite-form" class="form-grid"><label class="form-full">حساب المستخدم<select name="accountId" required>${accounts.length ? accounts.map((account) => `<option value="${escapeHtml(account.id)}">${escapeHtml(account.name)} · ${roleLabel(account.role)}</option>`).join("") : `<option value="" disabled>أضف حسابًا من إدارة المستخدمين أولًا</option>`}</select></label><div class="dialog__actions form-full"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit" ${accounts.length ? "" : "disabled"}>إنشاء الرمز</button></div></form>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
...
function openPairingRedeemDialog() {
  const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">ربط هذا الجهاز</span><h2>إدخال رمز الاقتران</h2><p class="dialog__subtext">أدخل الرمز الذي أنشأه الأدمن. بعد نجاح الربط يبقى هذا الجهاز مرتبطًا بحسابك فقط.</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><form id="pairing-redeem-form" class="form-grid"><label class="form-full">رمز الاقتران<input name="token" dir="ltr" inputmode="text" autocomplete="one-time-code" placeholder="معرّف المتجر:123456" required autofocus /></label><div class="dialog__actions form-full"><button class="button button--secondary" type="button" data-dialog-close>إلغاء</button><button class="button button--primary" type="submit">ربط الجهاز</button></div></form>`);
  overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
...
async function openCustomerTransaction(transaction) {
  ...
  if (transaction.type === "PAYMENT") {
    const overlay = openDialog(`<div class="dialog__head"><div><span class="eyebrow">إيصال دفعة عميل</span><h2>${escapeHtml(transaction.invoiceNumber || "دفعة عميل")}</h2><p class="dialog__subtext">${dateTime(transaction.date)}</p></div><button class="icon-button" data-dialog-close aria-label="إغلاق">${icon("close", 20)}</button></div><div class="invoice-detail"><div><span><strong>المبلغ المدفوع</strong><small>${escapeHtml(transaction.note || "تحصيل من العميل")}</small></span><strong>${money(Math.abs(toNumber(transaction.amount)))}</strong></div><div class="invoice-detail__final"><span>الرصيد بعد الدفعة</span><strong>${money(transaction.remainingAmount)}</strong></div></div><div class="dialog__actions"><button class="button button--primary" data-dialog-close>إغلاق</button></div>`);
    overlay.querySelectorAll("[data-dialog-close]").forEach((button) => button.addEventListener("click", closeDialog));
    return;
  }
